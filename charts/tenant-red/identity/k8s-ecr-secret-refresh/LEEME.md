...
```yaml
namespace: tenant-red-identity  # Cambia esto por tu namespace
```
...
### Ver el CronJob:
```bash
kubectl get cronjob -n tenant-red-identity
```

### Ver los Jobs ejecutados:
```bash
kubectl get jobs -n tenant-red-identity
```

### Ver el secret creado:
```bash
kubectl get secret eudistack-stg-ecr-secret -n tenant-red-identity
```
...
kubectl create job --from=cronjob/ecr-secret-refresh prueba-manual -n tenant-red-identity

# Ver los logs en tiempo real
kubectl logs -f job/prueba-manual -n tenant-red-identity

# Verificar que el secret se creó correctamente
kubectl describe secret eudistack-stg-ecr-secret -n tenant-red-identity
```
...
```bash
kubectl get jobs -n tenant-red-identity
kubectl logs job/ecr-secret-refresh-<id> -n tenant-red-identity
```

### Eliminar el CronJob:
```bash
kubectl delete cronjob ecr-secret-refresh -n tenant-red-identity
```
...
```bash
kubectl get serviceaccount ecr-secret-refresh-sa -n tenant-red-identity
kubectl get role ecr-secret-refresh-role -n tenant-red-identity
kubectl get rolebinding ecr-secret-refresh-rolebinding -n tenant-red-identity
```
